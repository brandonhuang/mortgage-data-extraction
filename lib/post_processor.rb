module PostProcessor
  def PostProcessor.split_assets(hash, assets_key)
  ...
  def PostProcessor.split_liabilities(hash, liab_key)
    hash[liab_key] = hash[liab_key].scan(/(.*?)(\$\s[\d,]+\.\d{2})\s(\$\s[\d,]+\.\d{2})\s(\$\s[\d,]+\.\d{2})/).map do |entry|
      {
        name: entry.first.strip,
